...
const World = {
    ...
    addEntity(type, x, z, chunk) {
        const prng = UI.Mulberry32(parseInt(this.mapSeed) + x * 11 + z * 3); // PRNG para aleatoriedade de entidades
        let entity;
        switch (type) {
            case 'exit':
                // Usa sprite 2D para a saída (porta)
                entity = new THREE.Sprite(new THREE.SpriteMaterial({ map: Game.assets.textures.get('exit'), fog: true, sizeAttenuation: true }));
                // Escala da porta: largura 80% da WALL_HEIGHT, altura igual à WALL_HEIGHT
                entity.scale.set(World.WALL_HEIGHT * 0.8, World.WALL_HEIGHT, 1);
            ...
            case 'item':
                const types = [
                    { type: 'food', texture: Game.assets.textures.get('food'), effect: s => s.hunger = Math.min(100, s.hunger + 40) },
                    { type: 'medkit', texture: Game.assets.textures.get('medkit'), effect: s => s.health = Math.min(100, s.health + 50) },
                    { type: 'pills', texture: Game.assets.textures.get('pills'), effect: s => s.sanity = Math.min(100, s.sanity + 30) },
                    { type: 'stone', texture: Game.assets.textures.get('stone'), effect: null }
                ], data = types[Math.floor(prng() * types.length)];
                entity = new THREE.Sprite(new THREE.SpriteMaterial({ map: data.texture, fog: true, sizeAttenuation: true }));
                // Escala do item: 25% da WALL_HEIGHT para largura e altura (quadrado)
                entity.scale.set(World.WALL_HEIGHT * 0.25, World.WALL_HEIGHT * 0.25, 1);
                // Posição do item: 25% da WALL_HEIGHT acima do chão
                entity.position.set(x, World.WALL_HEIGHT * 0.25, z);
                chunk.items.push({ ...data, sprite: entity });
                break;
            case 'stone':
                entity = new THREE.Sprite(new THREE.SpriteMaterial({ map: Game.assets.textures.get('stone'), fog: true }));
                // Escala da pedra: largura 50% da WALL_HEIGHT, altura 50% da WALL_HEIGHT
...
const Player = {
    ...
    // Verifica colisão do jogador em uma posição alvo
            ...
            if (e.button === 0) { // Clique esquerdo (usar item ou arremessar pedra)
                ...
                if (i) {
                    if (i.type === 'stone') Player.throwStone();
                        ...
                        i.effect(this.state.stats);
                        // Sons de uso de item
                        if (i.type === 'medkit') play8BitSound('medkit');
                        else if (i.type === 'pills') play8BitSound('pills');
                        else if (i.type === 'food') play8BitSound('eat');
                        else play8BitSound('use');
...
const Net = {
    ...
    init() {
        try {
            ...
            this.peer.on('open', id => {
                Player.state.id = id;
                UI.setPeerId(id);
                document.getElementById('my-peer-id').textContent = id; // Mostra o ID na tela
    ...
    setupConnection(c) {
        ...
        c.on('data', d => {
            ...
            if (p) {
                // Salva histórico das últimas 10 posições
                p.positionHistory.push({ pos: { ...d.pos }, time: Date.now() });
                if (p.positionHistory.length > 10) p.positionHistory.shift();

                // Se a posição recebida for muito diferente, faz rollback
                const dist = p.position.distanceTo(new THREE.Vector3(d.pos.x, d.pos.y, d.pos.z));
                if (dist > 10) { // 10 unidades é um exemplo, ajuste conforme seu jogo
                    // Rollback: volta para a posição recebida
                    p.sprite.position.set(d.pos.x, d.pos.y, d.pos.z);
    ...
    addRemotePlayer(id) {
        ...
        this.remotePlayers[id] = {
            ...
            nickname: '',
            nicknameMesh: null,
            positionHistory: [] // <-- Adicione isto
        };
        Engine.scene.add(s);
        // Cria nicknameMesh vazio (será atualizado ao receber dados)
        this.remotePlayers[id].nicknameMesh = createNicknameMesh('');
        Engine.scene.add(this.remotePlayers[id].nicknameMesh);
    },
...
const Game = {
    ...
    async loadAssets() {
        ...
        const assetsToLoad = {
            textures: [
                ['wall', 'assets/parede1.png'],
                ['floor', 'assets/chao1.jpg'],
                ['ceiling', 'assets/ceiling1.jpeg'],
                ['exit', 'assets/porta1.jpg'],
                ['monster', 'assets/monster1.png'],
                ['player', 'assets/player.webp'],
                ['food', 'assets/comida1.webp'],
                ['medkit', 'assets/medkit1.png'],
                ['pills', 'assets/remedio1.avif'],
                ['stone', 'assets/pedra1.webp'],
                ['jumpscare', 'assets/jumpscare1.gif']
            ]
        };
...
function play8BitSound(type) {
    ...
    function beep(freq, dur, vol, type = 'square', when = 0) {
        ...
        g.gain.value = vol * 2.0; // Volume reforçado
    ...
    switch (type) {
        case 'pickup':
            beep(1040, 0.09, 0.32, 'square'); beep(1560, 0.09, 0.22, 'square', 0.09);
            break;
        case 'damage':
            beep(180, 0.13, 0.38, 'square'); beep(90, 0.11, 0.22, 'square', 0.13);
            break;
        case 'jumpscare':
            beep(60, 0.18, 0.45, 'sawtooth'); beep(180, 0.12, 0.32, 'triangle', 0.18);
            break;
        case 'win':
            beep(660, 0.13, 0.32, 'square'); beep(880, 0.13, 0.32, 'square', 0.13); beep(1320, 0.18, 0.28, 'square', 0.26);
            break;
        case 'gameover':
            beep(220, 0.18, 0.32, 'triangle'); beep(110, 0.22, 0.28, 'triangle', 0.18);
            break;
        case 'footstep':
            beep(220, 0.05, 0.18, 'square');
            break;
        case 'monster':
            beep(90, 0.18, 0.22, 'sawtooth'); beep(60, 0.12, 0.16, 'sawtooth', 0.18);
            break;
        case 'throw':
            beep(400, 0.07, 0.22, 'square');
            break;
        case 'door':
            beep(300, 0.11, 0.22, 'triangle'); beep(200, 0.09, 0.16, 'triangle', 0.11);
            break;
        case 'use':
            beep(900, 0.08, 0.22, 'triangle');
            break;
        case 'medkit':
            beep(600, 0.09, 0.22, 'triangle'); beep(1200, 0.09, 0.18, 'triangle', 0.09);
            break;
        case 'pills':
            beep(800, 0.08, 0.22, 'triangle'); beep(1600, 0.08, 0.18, 'triangle', 0.08);
            break;
        case 'eat':
            beep(400, 0.09, 0.22, 'square'); beep(200, 0.09, 0.18, 'square', 0.09);
            break;
        case 'key':
            beep(1400, 0.07, 0.22, 'square'); beep(1800, 0.07, 0.18, 'square', 0.07);
            break;
    }
}
